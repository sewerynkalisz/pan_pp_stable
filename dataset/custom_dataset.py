...
train_split = os.path.join(root_dir, 'dataset_split', 'train.parquet')
train_data_dir = os.path.join(root_dir, 'images')
train_gt_dir = os.path.join(root_dir, 'annotations')
...
class CustomIC15Loader(data.Dataset):
    def __init__(self,
                 split='train',
                 is_transform=False,
                 img_size=None,
        ...
        self.df = pd.read_parquet(train_split)

        self.img_paths = [os.path.join(train_data_dir, img_path) for img_path in self.df.image_name.to_list()]
        self.gt_paths = [os.path.join(train_gt_dir, img_path) for img_path in self.df['gt'].to_list()]
