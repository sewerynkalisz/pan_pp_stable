...
from dataset import CustomIC15Loader
import models
...
def save_checkpoint(args, state, checkpoint='checkpoint',
    ...
    epoch = state['epoch']
    iter = state['iter']
    if iter == 0 and epoch > args.epoch - 100 and epoch % 5 == 0:
...
def main(args):
    if args.checkpoint == '':
        args.checkpoint = 'checkpoints/custom_{arch}_{img_size}'.format(
            arch=args.arch,
